...
//        return view('engineer.bug.index')->with('data',$bugs);
        $data = DB::select("select project.pid,project.pname from project where project.cpid=:uid and project.pstatus='0'",
            ['uid'=>session('userid')]);
        return view('cp.bug.index',compact('data'));
    }
    ...
    public function create()
    {
        $data =  DB::select('select project.pid,project.pname from project,userproject where userproject.uid=:uid and userproject.status=1 and userproject.pid=project.pid',
            ['uid'=>session('userid')]);
        return view('cp.bug.add',compact('data'));
    }
    //get.engineer/bug/{bug}    显示单个bug信息
    public function show($bug_id)
    {
        $field = Bug::find($bug_id);
        $data =  DB::select('select project.pid,project.pname from project where cpid=:uid',['uid'=>session('userid')]);
        return view('cp/bug/show', compact('field','data'));
    }
    ...
    public function edit($bug_id)
    {
        $field = Bug::find($bug_id);
        $data =  DB::select('select project.pid,project.pname from project where cpid=:uid',['uid'=>session('userid')]);
        return view('cp/bug/edit', compact('field','data'));
    }
    ...
    public function getbug()
    {
        ...
        $rs = mysql_query("select count(*) from bug,user,project where ".$query_keyword."project.cpid='$uid'".$query_user."$query_project"." and bug.uid=user.uid and bug.pid=project.pid order by bug.btime asc");
        $row = mysql_fetch_row($rs);
        $result["total"] = $row[0];
        //$rs = mysql_query("select case_mmorpg.cid,cmodel,ccase,cexpect,ctype,cresult,cbug from $pname limit $offset,$rows");
        $rs= mysql_query("select bug.bid,bug.btitle,bug.uid,bug.pid,bug.bdescription,bug.binarydata,bug.binarydata2,bug.status,bug.btime,project.pname,user.username from bug,user,project where ".$query_keyword."project.cpid='$uid'".$query_user."$query_project"." and bug.uid=user.uid and bug.pid=project.pid order by bug.btime desc limit $offset,$rows");
        $items = array();
