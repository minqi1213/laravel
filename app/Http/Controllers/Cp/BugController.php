...
class BugController extends CommonController
{
    public function index()
    {
        $data = DB::select("select project.pid,project.pname from project where project.cpid=:uid and project.pstatus='0'",
            ['uid'=>session('userid')]);
        return view('cp.bug',compact('data'));
    }
    ...
    public function getbug()
    {
        ...
        $rs= mysql_query("select bug.bid,bug.btitle,bug.uid,bug.pid,bug.bdescription,bug.binarydata,bug.binarydata2,bug.btime,project.pname,user.username from bug,user,project where ".$query_keyword."project.cpid='$uid'".$query_user."$query_project"." and bug.uid=user.uid and bug.pid=project.pid order by bug.btime asc limit $offset,$rows");
        $items = array();
